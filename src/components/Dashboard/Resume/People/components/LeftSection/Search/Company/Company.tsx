...
function Company() {
  ...
  const [isNotAnyOf, setIsNotAnyOf] = useState(
    searchData.company_not_in_names.length ? true : false
  );
  ...
  const [isPastJobTitle, setIsPastJobTitle] = useState(
    searchData.company_past_names.length ? true : false
  );
  ...
  const defCompanyData = searchData.company_names.map((name: any) => {
    return { name: name, count: 999 };
  });
  const [defCompany, setDefCompany] = useState(defCompanyData);

  const defNotCompanyData = searchData.company_not_in_names.map((name: any) => {
    return { name: name, count: 999 };
  });
  const [defNotCompany, setDefNotCompany] = useState(defNotCompanyData);

  const defPastCompanyData = searchData.company_past_names.map((name: any) => {
    return { name: name, count: 999 };
  });
  const [defPastCompany, setDefPastCompany] = useState(defPastCompanyData);

  const defExcludeCompanyData = searchData.exclude_company_names.map(
    (name: any) => {
      return { name: name, count: 999 };
    }
  );
  ...
  React.useEffect(() => {
    if (searchData.company_names === "") {
      setDefCompany("");
    }
    if (searchData.company_not_in_names === "") {
      setDefNotCompany("");
    }
    if (searchData.company_past_names === "") {
      setDefPastCompany("");
    }
    if (searchData.exclude_company_names === "") {
      setDefExcludeCompany("");
    }
  }, [
    searchData.company_names,
  ...
  React.useEffect(() => {
    // getSearchData("");
    if (searchData.exist_fields.includes("company")) {
      setIsUnKnown(false);
      setIsAnyOf(false);
      setIsKnown(true);
      setIsNotAnyOf(false);
      setIsPastJobTitle(false);
    } else if (searchData.not_exist_fields.includes("company")) {
      setIsUnKnown(true);
      setIsAnyOf(false);
      setIsKnown(false);
      setIsNotAnyOf(false);
