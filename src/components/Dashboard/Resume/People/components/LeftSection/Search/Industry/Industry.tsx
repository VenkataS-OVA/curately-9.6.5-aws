...
const Industry = () => {
  ...
  const onClickCompanyKeywords = () => {
    ...
    setIsInclude(true);
    if (searchData.industry_all_company_names.length !== 0) {
      setIsAll(true);
    } else {
      setIsAll(false);
    }
    if (searchData.industry_company_not_in_names.length !== 0) {
      setIsExclude(true);
    } else {
      setIsExclude(false);
    }
  ...
  const defIndustryData = searchData.industries.map((name: any) => {
    return { name: name, count: 999 };
  });
  const [defIndustry, setDefIndustry] = React.useState(defIndustryData);

  const defNotIndustryData = searchData.industries_not_in.map((name: any) => {
    return { name: name, count: 999 };
  });
  ...
  const defExcludeCompanyIndustryData =
    searchData.industry_company_not_in_names.map((name: any) => {
      return { name: name, count: 999 };
    });
  const [defExcludeCompanyIndustry, setDefExcludeCompanyIndustry] =
    React.useState(defExcludeCompanyIndustryData);

  const defIncludeCompanyIndustryData = searchData.industry_company_names.map(
    (name: any) => {
      return { name: name, count: 999 };
    }
  );
  ...
  const defIncludeAllCompanyIndustryData =
    searchData.industry_all_company_names.map((name: any) => {
      return { name: name, count: 999 };
    });
  ...
  const handleExcludeAutocompleteChange = (event: any, value: any) => {
    ...
    if (value.length > 0) {
      ...
      const updatedIndustry = searchData.industries_not_in
        .filter((industry: string) => !selectedOptions.includes(industry))
        .concat(selectedOptions);
  ...
  const handleExcludeCompanyAutocompleteChange = (event: any, value: any) => {
    ...
    if (value.length > 0) {
      ...
      const updatedIndustry = searchData.industry_company_not_in_names
        .filter((industry: string) => !selectedOptions.includes(industry))
        .concat(selectedOptions);
  ...
  const handleIncludeCompanyAutocompleteChange = (event: any, value: any) => {
    ...
    if (value.length > 0) {
      ...
      const updatedIndustry = searchData.industry_company_names
        .filter((industry: string) => !selectedOptions.includes(industry))
        .concat(selectedOptions);
  ...
  const handleIncludeAllCompanyAutocompleteChange = (
    ...
    if (value.length > 0) {
      ...
      const updatedIndustry = searchData.industry_all_company_names
        .filter((industry: string) => !selectedOptions.includes(industry))
        .concat(selectedOptions);
  ...
  React.useEffect(() => {
    ...
    if (searchData.industries_not_in === "") {
      setDefNotIndustry([]);
    }
    if (searchData.industry_company_not_in_names === "") {
      setDefExcludeCompanyIndustry([]);
    }
    if (searchData.industry_company_names === "") {
      setDefIncludeCompanyIndustry([]);
    }
    if (searchData.industry_all_company_names === "") {
      setDefIncludeAllCompanyIndustry([]);
    }
  }, [
    searchData.industries,
    searchData.industries_not_in,
    searchData.industry_company_not_in_names,
  ...
  React.useEffect(() => {
    getSearchData("");
    if (searchData.industries_not_in.length !== 0) {
      setIsNotAnyOf(true);
      setIsAdvSetting(true)
    } else if (searchData.industry_adv_settings === "known") {
      setIsKnown(true);
      setIsNotAnyOf(false);
      setIsAdvSetting(true)
    } else if (searchData.industry_adv_settings === "unKnown") {
      setIsUnKnown(true);
      setIsNotAnyOf(false);
      setIsAdvSetting(true)
    }
    ...
    if (searchData.industry_company_names.length !== 0) {
      setIsInclude(true);
    }
    if (searchData.industry_all_company_names.length !== 0) {
      setIsAll(true);
