...
const TotalYearsOfExperienceModal = () => {
  const [searchModalData, setSearchModalData] = useContext(ModalStore);

  const [minYear, setMinYear] = useState<any>(searchModalData.minYear);
  const [maxYear, setMaxYear] = useState<any>(searchModalData.maxYear);
  ...
  const handleChangeMinYear = (e: any) => {
    ...
    if (stringValue.length === 0 || searchModalData.maxYear.length === 0) {
      setSearchModalData((prevSearchData: any) => ({
        ...prevSearchData,
        isMinYearValidation: false,
        isMaxYearValidation: false,
  ...
  const handleChangeMaxYear = (e: any) => {
    ...
    if (stringValue.length <= 2) {
      ...
      const currMinYear = searchModalData.minYear;
      if (currMinYear) {
    ...
    if (stringValue.length === 0 || searchModalData.minYear.length > 0) {
      setSearchModalData((prevSearchData: any) => ({
        ...prevSearchData,
        maxYear: e.target.value,
      }));
  ...
  useEffect(() => {
    if (searchModalData.minYear === "") {
      setMinYear("");
    }
    if (searchModalData.maxYear === "") {
      setMaxYear("");
    }
  ...
  return (
    ...
    (<Stack>
      ...
      >
        ...
        {searchModalData.isMinYearValidation && (
          <Typography
            sx={{
              fontSize: "12px",
              fontWeight: "400",
        ...
        {searchModalData.isMaxYearValidation && (
          <Typography
            sx={{
              fontSize: "12px",
              fontWeight: "400",
...
export default TotalYearsOfExperienceModal;
