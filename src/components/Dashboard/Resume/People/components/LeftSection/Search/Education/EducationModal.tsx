...
const EducationModal = () => {
  ...
  const [fromToYearEduValidation, setFromToYearEduValidation] = useState(false);

  const [toFromYearEduValidation, setToFromYearEduValidation] = useState(false);

  const [startYear, setStartYear] = useState(
    searchModalData.education?.educationStartYear ? searchModalData.education?.educationStartYear : ""
  );
  const [endYear, setEndYear] = useState(
    searchModalData.education?.educationEndYear ? searchModalData.education?.educationEndYear : ""
  );
  ...
  const handleChangeStartYear = (event: any) => {
    ...
    if (stringValue.length === 4) {
      ...
      if (endDate.isAfter(today)) {
        ...
        if (endYear) {
          if (
            moment(endYear).isAfter(moment(stringValue)) ||
            moment(endYear).isSame(moment(stringValue))
          ) {
  ...
  //   searchModalData.education.degreeIn?.splice(index, 1);
  //   setSearchModalData((prevSearchData: any) => ({
  //     ...prevSearchData,
  //     education: {
  //       ...searchModalData.education,
  //       degreeIn: searchModalData.education.degreeIn,
  //     },
  //   }));
  // };
  ...
  //   console.log("options", option, index);
  //   searchModalData.education.schoolIn?.splice(index, 1);
  //   setSearchModalData((prevSearchData: any) => ({
  //     ...prevSearchData,
  //     education: {
  //       ...searchModalData.education,
  //       schoolIn: searchModalData.education.schoolIn,
  //     },
  //   }));
  // };
  ...
  //   console.log("options", option, index);
  //   searchModalData.education.majorIn?.splice(index, 1);
  //   setSearchModalData((prevSearchData: any) => ({
  //     ...prevSearchData,
  //     education: {
  //       ...searchModalData.education,
  //       majorIn: searchModalData.education.majorIn,
  //     },
  //   }));
  // };

  const defDegreeData = searchModalData.education?.degreeIn?.map(
    (name: any) => {
  ...
  const handleChangeDegree = (event: any, value: any) => {
    console.log("degree modal value", value);
    // debugger
    setSearchModalData((prevSearchData: any) => ({
      ...prevSearchData,
      eduDegreeList: value,
    ...
    if (value.length > 0) {
      ...
      const updatedDegreeTitles = searchModalData.education?.degreeIn
  ...
  const defSchoolData = searchModalData.education?.schoolIn ? searchModalData.education?.schoolIn?.map((name: any) => {
    return { name: name, count: 999 };
  }) : [];
  ...
  const handleChangeSchool = (event: any, value: any) => {
    ...
    if (value.length > 0) {
      ...
      const updatedSchoolTitles = searchModalData.education?.schoolIn
        .filter((title: string) => !selectedOptions.includes(title))
        .concat(selectedOptions);
  ...
  const defMajorData = searchModalData?.education?.majorIn ? searchModalData.education.majorIn.map((name: any) => {
    return { name: name, count: 999 };
  }) : [];
  ...
  const handleChangeMajor = (event: any, value: any) => {
    ...
    if (value.length > 0) {
      ...
      const updatedMajorTitles = searchModalData.education?.majorIn
        .filter((title: string) => !selectedOptions.includes(title))
        .concat(selectedOptions);
  ...
  React.useEffect(() => {
    if (searchModalData.eduDegreeList.length === 0) {
      setDefDegree([]);
    } else {
      setDefDegree(searchModalData.eduDegreeList);
    }
    if (searchModalData.eduSchoolList.length === 0) {
      setDefSchool([]);
    }
    if (searchModalData.eduMajorList.length === 0) {
      setDefMajor([]);
    }

    if (searchModalData.education?.educationStartYear === "") {
      setStartYear("");
    } else {
      setIsYearOfEducation(true);
    }
    if (searchModalData.education?.educationEndYear === "") {
      setEndYear("");
    } else {
      setIsYearOfEducation(true);
    }

    if (searchModalData.education?.degreeIn?.length >= 1) {
      setIsDegree(true);
    }

    if (searchModalData.education?.schoolIn?.length >= 1) {
      setIsSchool(true);
    }

    if (searchModalData.education?.majorIn?.length >= 1) {
      setIsMajor(true);
    }
  }, [searchModalData.education]);

  // console.log('zzzzzzzzzz', searchModalData.education)
                  ...
                  control={
                    <BpCheckboxContainer>
                      <Checkbox
                        ...
                        checked={
                          isDegree || searchModalData.eduDegreeList.length > 0
                        }
                      />
                    </BpCheckboxContainer>
                  }
                  ...
                  control={
                    <BpCheckboxContainer>
                      <Checkbox
                        ...
                        checked={
                          isSchool || searchModalData.eduSchoolList.length > 0
                        }
                      />
                    </BpCheckboxContainer>
                  }
