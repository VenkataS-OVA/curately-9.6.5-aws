...
import { checkPermision } from "../../../../shared/utills/helper";
...
const Persona: React.FC<PersonaProps> = (
  ...
  // useEffect(() => {
  //   refForUpdatingData.current = updatePersonaDataFromModal;
  // }, [])

  // const updatePersonaDataFromModal = (data: any) => {
  //   setPersonaData(data);
  // }
  // console.log(checkPermision("400025"), 'hello AP ')
  ...
  const [personaChecked, setPersonaChecked] = React.useState<{ [key: string]: boolean }>({});

  const [personadata, setPersonaData] = React.useState<any[] | never[]>([]);
  const [managePersonadata, setManagePersonaData] = React.useState<[] | null>(null)
  ...
  const handlePersonaCheckboxChange = (persona: any) => {
    setPersonaChecked((prevCheckItems: any) => ({
      ...prevCheckItems,
      [persona.personaId]: !prevCheckItems[persona.personaId],
    }));
    ...
    // if (searchData.personaIds.includes(personaCheckedLabelName)) {
    //   // const filterPersonaChecked = searchData.personaIds.filter((item: any) => item === personaCheckedLabelName)

    //   const labelIndex = searchData.personaIds.indexOf(personaCheckedLabelName)

    //   const personaRemove = searchData.personaIds.splice(labelIndex, 1)

    //   // console.log('labelIndex', labelIndex)

    //   setSearchData((prevSearchData: any) => ({
    //     ...prevSearchData,
    //     personaIds: prevSearchData.personaIds
    //   }));

    // } else {
    //   setSearchData((prevSearchData: any) => ({
    //     ...prevSearchData,
    //     personaIds: [...prevSearchData.personaIds, personaCheckedLabelName],
    //   }));
    // }
    ...
    setSearchData((prevSearchData: any) => {
      const updatedPersonaIds = prevSearchData.personaIds.includes(persona.personaId)
        ? prevSearchData.personaIds.filter((id: number) => id !== persona.personaId)
        : [...prevSearchData.personaIds, persona.personaId]; // Add only the personaId for consistency

      const updatedCheckedPersonas = prevSearchData.checkedPersonas.some(
        (p: any) => p.personaId === persona.personaId
      )
        ? prevSearchData.checkedPersonas.filter(
          (p: any) => p.personaId !== persona.personaId
        )
        : [...prevSearchData.checkedPersonas, persona];

      return {
        ...prevSearchData,
        personaIds: updatedPersonaIds,
  ...
  return (
    ...
    (<Stack sx={{ mb: 1 }}>
      ...
      >
        ...
        <Box >
          <Stack sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', ml: 1 }}>

            <Stack sx={{
              display: 'flex', flexDirection: 'row', alignItems: 'center', cursor: 'pointer',
              color: styles.primaryTextColor, ml: '5px',
              '&:hover': {
            ...
            <>
              {personadata.filter((data: any) => data.isActive).map((item: any) => (
                ...
                >
                    ...
                    <Box >
                      <BpCheckboxContainer >
                        <Checkbox
                          ...
                          color="default"
                          checked={
                            searchData.personaIds.includes(
                              item.personaId
                            )
                              ? true
                              : false
                          ...
                          // checked={searchData.personaIds.includes([item.personaId])}
                          checkedIcon={
                            <BpCheckedIcon
                              // className="bp-icon"
                              style={{
                          ...
                          onChange={() =>
                            handlePersonaCheckboxChange(item)
                          }
                        />
                      </BpCheckboxContainer>
                    </Box>

                    <Box component="label" htmlFor={item.personaId}
                  ...
                  {(isEditPersona === item.personaId && checkPermision("400025")) &&
                    <>
                      <Box component='div'
                        sx={{ width: '8%', pr: 4 }}
            ...
            <Stack sx={{ borderBottom: '1px solid #e6e6e6', mt: 1, mb: 1, width: '98%' }}></Stack>

            <Stack sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '98%' }} >
              ...
              <Button
                ...
                startIcon={<AddCircleOutlineRoundedIcon sx={{ color: "#146ef6" }} />}
                sx={{
                  width: '45%',
                  height: '32px',
                  borderColor: "rgba(202, 202, 204, 1)",
