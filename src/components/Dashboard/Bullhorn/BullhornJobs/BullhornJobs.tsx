...
const BullhornJobs = () => {
    ...
    const [searchJobName, setSearcheJobName] = useState<any>('');
    const [searchCompanyName, setSearchCompanyName] = useState<any>('');
    const [bullhornJobsList, setBullhornJobsList] = useState<any[] | never[]>([]);
    ...
    const columns: MRT_ColumnDef<any>[] = [
        ...
        {
            accessorKey: "clientCorporation.name",
            accessorFn: (row: any) => `${row.clientCorporation.name} `,
            header: "Company Name",
        },
        {
    ...
    const flatData = useMemo(
        () => {
            return bullhornJobsList?.filter((page) => {
                return page.title.toLowerCase().includes(searchValue.toLowerCase() || "");
            }) ?? [];
        },
        [searchValue, bullhornJobsList],
    );


    const getBullhornJobsList = () => {
        if (searchJobName || searchCompanyName || (startDate && endDate && (startDate.startOf("day") <= endDate.startOf("day")))) {
            ...
            let data = {
                ...
                "fields": "id,title,dateLastModified,clientCorporation"
            }
    ...
    const JobPushBullhorntoCurately = () => {
    ...
    useEffect(() => {
        if (searchJobName || (startDate && endDate && (startDate.startOf("day") <= endDate.startOf("day")))) {
            getBullhornJobsList();
        }
    ...
    const handleSelectAll = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.target.checked) {
            let rowData: any = {};
            let tempData: any = bullhornJobsList;
            for (let index = 0; index < bullhornJobsList.length; index++) {
                ...
                >
                    <Stack direction="row" spacing={3} style={{ paddingRight: '22px' }}>
                        <TextField
                            ...
                            value={searchJobName}
                            onChange={(e) => setSearcheJobName(e.target.value)}
                        />
                    </Stack>
                    <Stack direction="row" spacing={3} style={{ paddingRight: '22px' }}>
                        <TextField
                    ...
                    {searched ? <div className="MRTableCustom pl-0 ">
                        ...
                        <Grid container className="actionItems" width={"auto"}>
                            ...
                            <Button
                                ...
                                onClick={() => JobPushBullhorntoCurately()}
                            >
                                Push to Curately
                            </Button>
                        </Grid>
