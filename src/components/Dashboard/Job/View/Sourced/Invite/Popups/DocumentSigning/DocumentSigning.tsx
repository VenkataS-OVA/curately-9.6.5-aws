...
import { URL } from "../../../../../../Letters/Workflow/FormBuilder/shared/utills/Constants";
...
const DocumentSigning = ({
    ...
    useEffect(() => {
        const getDocumentInfo = () => {
            ...
            //commented for future purpose//
            trackPromise(
                APIService.getByParams(193, 'Curately/Invite/invite_job_get.jsp', postData).then((response) => {
                    setTemplateData(response.data.documentsigning)
                        ...
                        finalFormFields.forEach((component: any) => {
                            if (!component.choices) {
                                fieldsArr.push({
                                    ...
                                    required: component.isRequired ? component.isRequired : false,
                                })
                            }
                            else {
                                fieldsArr.push({
    ...
    const assignDataCollection = () => {
        const { selectedRows, inviteToAssignCB, updated } = inviteProps
        let userIds = Object.keys(selectedRows)
        if (!!userIds?.length && userIds.length > 1) {
            userIds.forEach((userId) => {
                inviteToAssignCB(5, userId);
            })
        } else {
            inviteToAssignCB(5);
            updated("5");
    ...
    return (
        <Grid container>
            {loader ? <Box sx={{ width: "100%", minHeight: "150px", display: "flex", justifyContent: "center", alignItems: "center" }}><CircularProgress /></Box> :
            ...
            <DocumentModal open={isOpen} onClose={handleClose} documentId={documentId} handleRefresh={handleRefresh} formData={formFields} documentName={templateData?.documentName} stageId={inviteProps.stageId} />
            ...
            {templateData && <Stack direction={"row"} sx={{ width: "100%", position: "relative", top: "12px" }} justifyContent={"flex-end"} alignItems={"center"} spacing={1} my={1} mr={1}>
                <Button variant='contained' disableElevation onClick={assignDataCollection}>Assign</Button>
            </Stack>}
