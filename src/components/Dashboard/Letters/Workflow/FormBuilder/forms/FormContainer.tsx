...
const FormContainer = (
    { formIdPassed,
        ...
        customFields,
        isCustomFieldDeleteSettingEnabled = true
        // initialFormData
    }
        :
        {
    ...
    const addFormElement = (type: any) => {
        ...
        console.log("addingPositionTemp : " + addingPositionTemp.current);
    ...
    const copyFormElement = (type: any, index: number) => {
        ...
        obj.value = "";
        let tempChoice: any[] = [];
        if (obj.fieldType === "weightedmultiplechoice" || obj.fieldType === "multiplechoice") {
            let count = 1;
            obj.choices.forEach((choice: any) => {
                const newChoice = { ...choice };
                const newId = count + obj.id;
                newChoice.id = newId;
                tempChoice.push(newChoice);
                count++;
            })
        }

        obj.choices = [...tempChoice];
        ...
        if (!formLabelEmpty) {
            ...
            if (weightedArray.length > 0) {
                for (let wa = 0; wa < weightedArray.length; wa++) {
                    ...
                    if (!('weightedPoints' in item) || item['weightedPoints'] === "") {
                        setFocusId(item.id);
                        showToaster(`Please enter points for ${item.labelName}`, 'error');
                        break;
                    } else if (!('weightedCorrectAnswer' in item) || item['weightedCorrectAnswer'] === "") {
                        setFocusId(item.id);
            ...
            <Grid sx={{ flexGrow: 1 }}>
                <Grid>
                    ...
                    >
                        ...
                        <Box>
                            <Header setFocusId={(val: any) => setFocusId(val)} customFields={customFields} />
                        </Box>
                        <Box className='d-flex'>
                            {
                                showSave ?
                                    <Button size="small" variant='contained' color='primary' className="form-action-btn" sx={{ mr: 1 }} onClick={()=>{saveData(); saveAuditLog(4279);}} disabled={formData.length === 0}>{formIdPassed ? "Update" : "Save"}</Button>
                                    :
                                    null
                            }
                            {
                                showCancel ?
                                    <Button size="small" variant='outlined' color='secondary' className="form-action-btn" onClick={cancelClicked}>Cancel</Button>
                                    :
                                    null
                            }
                        </Box>
