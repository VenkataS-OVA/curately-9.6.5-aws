...
import { useForm } from "../../../../../../shared/modules/ReactHookForm"; // Changed import
import {
	ToggleButtonGroup,
	ToggleButton,
} from "../../../../../../shared/modules/MaterialImports/ToggleButton";
...
const PreviewComponent = ({
	...
	isPreview: boolean;
	saveCandidateData: any;
	isShowOneByOne: boolean;
	formAnswers?: any;
}) => {
	...
	const [currPageElements, setCurrPageElements] = useState([]);
	...
	// const [localFieldsState, setLocalFieldsState] = useState(null);
		...
		switch (conditionVal) {
			case "less than":
				if (years === yearsToCompare) {
					if (actualMonth < validateMonth) isValidateDate = true;
				} else {
					if (years < yearsToCompare) {
						isValidateDate = true;
					}
	...
	const validateDate = (data: any, formValues: any) => {
		...
		if (conditions.value && otherFields.value) {
			switch (conditions.value[0]) {
				case "Less than(<)": {
					let dateValue = formValues[data.id.toString()];
					let checkObj = formData.filter(
						(dataVal: any) => dataVal.labelName === otherFields.value[0]
					);
					if (checkObj) {
						toCompareDate = formValues[checkObj[0]?.id?.toString()];

						if (customDateRanges.indexOf(otherFields.value[0]) === -1) {
							dateErrObj.isValid =
								new Date(dateValue) < new Date(toCompareDate) ? true : false;
							dateErrObj.errorText = `${data.labelName} should be less than ${otherFields.value} `;
						} else {
							const customDateVal = getCustomDateValidation(
								dateValue,
								toCompareDate,
								otherFields.value[0],
								"less than"
				...
				case "Equals(=)": {
					...
					if (checkObj) {
						...
						if (customDateRanges.indexOf(otherFields.value[0]) === -1) {
							dateErrObj.isValid =
								new Date(dateValue).getTime() === new Date(toCompareDate).getTime()
									? true
									: false;
	...
	const validationData = (formValues: any) => {
					...
					if (obj.isRequired) {
						switch (obj.fieldType) {
							case "phone":
								if (obj.isValidate) {
									acc[obj.id] = yup
										...
										.min(12, "Phone number should be min 10 characters")
										.test(
											"isValidPhoneNumber",
							...
							case "email":
								...
								break;
							case "date": {
								let [conditions, _] = obj.dateConditions?.dependencyObjs;
								if (conditions.value) {
									acc[obj.id] = yup
										.date()
										.test(
											"isValidDate",
											validateDate(obj, formValues).errorText,
											() => {
												return validateDate(obj, formValues).isValid;
											}
										)
										.required(obj.labelName + " is required.");
								} else {
								...
								if (!obj.enableAddressValidation) {
									...
									acc[obj.id] = yup
										...
										.test(
											...
											async (value: any, ctx: any) => {
												if (
													value.full_address &&
													value.city &&
													value.state
												) {
													let errorPromise = await smartyStreetAddress(
														value
													);
													if (errorPromise?.isError) {
														return ctx.createError({
															message: errorPromise?.errorText,
														});
													}
												}
												return true;
							...
							case "textbox":
								if (obj.maxCharacter) {
									acc[obj.id] = yup
										...
										.required(obj.labelName + " is required.")
										.max(
											Number(obj.maxCharacter),
											`${obj.labelName} characters length should be equal to ${obj.maxCharacter}`
										);
									...
									switch (obj.allowedInputs[0]) {
										case "Numeric with decimal":
											pattern = /^\d+$|^\d+\.\d{0,2}$/;
											errorMessage =
												"Please enter only Numeric values with decimal points";
											break;
										case "Text":
											pattern =
												/^[a-zA-Z`~!@#$%^&*()_+-=[\]\;',.\/{}|:"<>?\s]+$/;
											errorMessage =
												"Please enter only text input with alphabatical values";
											break;
										case "Numeric":
											pattern = /^[0-9]+$/;
							...
							case "checkbox":
								acc[obj.id] = yup
									.boolean()
									.oneOf([true], `${obj.labelName} is required`);
								break;
							default:
								acc[obj.id] = yup
									.string()
	...
	const validationSchema = useMemo(() => {
		return yup.object().shape(validationData(formData));
	}, [formData]);
	...
	const {
		...
		resolver: yupResolver(validationSchema, { context: formData }),
	});

	const handleOpenCodeVerifyModal = (value: any, phone: any, formId: any) => {
		setValue(formId.toString(), phone);
		setModalValue(phone);
		setOtp(value);
	...
	const handleOpenVerifyModal = (item: any) => {
		setModalValue(getValues(item.id.toString()));
	...
	const shouldDisplayBlock = (formData: any, formItem: any) => {
		const conditionalField = formData.find(
			(item: any) => item.fieldType === "conditionallogic"
		);
		...
		const selectedCondition =
			conditionalField?.settings?.when?.conditionalList?.selectedList[0];
		const selectedValue =
			conditionalField?.settings?.when?.values?.selectedList?.[0] ||
			conditionalField?.settings?.conditions?.[0]?.values?.selectedList?.[0];
		const action =
			conditionalField?.settings?.then?.[0]?.mainList?.selectedList?.[0]?.toLowerCase();
		const dependentFieldId = conditionalField?.settings?.when?.mainList?.selectedList?.[0]?.id;
		let expectedValue = getValues(dependentFieldId?.toString()) || "";
		...
		const isConditionMet = (condition: any) => {
			...
			const actualSubValue = getValues(subDependentFieldId?.toString()) || "";

			if (!subSelectedCondition) return true;

			const normalizedSubValue = Array.isArray(actualSubValue)
				? actualSubValue[0]
				: actualSubValue;
		...
		const areAllConditionsMet = (conditionsList: any[]) => {
		...
		const mainList =
			conditionalField?.settings?.conditions?.[0]?.mainList?.selectedList?.[0]?.label || "Or";
		...
		if (
			finalConditionMet &&
			components.some((component: any) => component.id === formItem.id)
		) {
			const isHideAction = action === "hide blocks";
			const isShowAction = action === "show blocks";
			if (isHideAction) {
				return !components.some((component: any) => component.id === formItem.id);
			}
	...
	const onSaveChangesCalled = (errors = {}) => {
		setIsForward(true);
		...
		let data = formData.filter(
			(form: any) =>
				HIDE_COMPONENTS.indexOf(form.fieldType) === -1 &&
				form.displayName !== "Conditional Logic"
		);
		...
		if (data.length === tempAct || !isShowOneByOne || data.length === currentSelectedIndex) {
			...
			let resultToPass = Object.keys(getValues()).map((key) => ({
				quesId: key,
				answer: getValues(key),
				quesType: "",
			}));
			let dataToSaveForDB = Object.keys(getValues()).map((key) => ({
				question: "",
				quesId: key,
				answer: getValues(key),
				datakey: "",
			...
			for (let tf = 0; tf < resultToPass.length; tf++) {
				let tempObj = formData.find(
					(c: any) => c.id?.toString() === resultToPass[tf]?.quesId?.toString()
				);
				if (tempObj?.inputType === "label") {
					...
					if (tempObj?.inputType === "fileupload") {
						...
						tempObj?.fieldType === "address" ||
						tempObj?.fieldType === "rating"
					) {
						...
						resultToPass[tf].answer = JSON.stringify(
							resultToPass[tf].answer?.toString()
						);
						dataToSaveForDB[tf].answer = JSON.stringify(
							resultToPass[tf].answer?.toString()
						);
					}
				}
			...
			if (!isPreview && !Object.keys(errors).length) {
				saveCandidateData(resultToPass, dataToSaveForDB);
			}
			if (!isShowOneByOne && Object.keys(errors).length) {
	...
	const splitDataById = () => {
		let inc = 0;
		let finalAry: any[] = [[]];
		let finalData = formData.filter(
			(item: any) => item.fieldType !== "conditionallogic" && item.isShow
		);
	...
	useEffect(() => {
		if (!isShowOneByOne) {
			let finalArray: any = splitDataById();
	...
	const generateStars = (len: number) => {
		return "*".repeat(len);
	};
	...
	const handleChange = (event: any, id: any): void => {
		...
		const updatedFormData = formData.map((field: any) => {
			...
			setValue(id?.toString(), generateStars(updatedValue.length), { shouldValidate: true });
	...
	const showActValue = (id: any): void => {
		const artwork = formData.find((field: any) => field.id === id);
		if (!artwork) return;

		setTimeout(() => {
			setValue(id.toString(), artwork.actVal, { shouldValidate: true });
			setFormData([...formData]);
		}, 100);
	};

	const showStarValue = (id: any): void => {
		const artwork = formData.find((field: any) => field.id === id);
		...
		setTimeout(() => {
			setValue(id.toString(), stars, { shouldValidate: true });
			setFormData([...formData]);
		}, 100);
	};
	...
	const getFile = (file: any, id: any) => {
		setFlag((prev) => !prev);
		setIdVal(id);
		setValue(id.toString(), file, { shouldValidate: true });
	};

	const getDateValue = (dateVal: any, formId: any) => {
		setFlag((prev) => !prev);
		setIdVal(formId);
		setValue(formId.toString(), dateVal?.toFormat("MM/dd/yyyy"), { shouldValidate: true });
	};
	...
	const formatPhoneNumber = (value: any) => {
		...
		if (!value) {
			...
			phoneNumValue = `${phoneNum.slice(0, 3)}-${phoneNum.slice(3, 6)}-${phoneNum.slice(
				6,
				10
			)}`;
		}
	...
	const getAddressValue = (addressObj: any, formId: any) => {
		setFlag((prev) => !prev);
		setIdVal(formId);
		setValue(formId.toString(), addressObj, { shouldValidate: true });
	};

	const handleRadioChange = (e: any, form: any) => {
		setValue(form.id.toString(), e.target.value, { shouldValidate: true });
	};
	...
	const handleCheckboxChange = (e: any, form: any) => {
		if (e?.target?.checked) {
			setValue(
				form.id?.toString(),
				[...(getValues(form.id?.toString()) || []), e?.target?.value],
				{
					shouldValidate: true,
				}
			);
		} else {
			let temArr = getValues(form.id?.toString()) || [];
			const newArry = temArr.splice(0, temArr.indexOf(e?.target?.value));
			setValue(form.id.toString(), [...newArry], { shouldValidate: true });
		}
	};

	const getSelectedValue = (value: any, id: any) => {
		setValue(id.toString(), value || "", { shouldValidate: true });
		setFlag((prev) => !prev);
		setIdVal(id);
	};
	...
	const getRankValues = (value: any, id: any) => {
		if (value) {
			...
			setValue(id.toString(), serializedValue, { shouldValidate: true });
		} else setValue(id.toString(), "");
		setIdVal(id);
		setFlag((prev) => !prev);
	};
	...
	const evaluateCheckboxCondition = (condition: string, values: any) => {
		return ["isfilled", "ischecked", "is"].includes(condition)
			? !!values
			: condition === "isblank"
			? !values
			: false;
	...
	const evaluateCondition = (condition: string, values: any, valueToCheck: any) => {
		switch (condition) {
			case "isfilled":
				return !!values;
			case "is":
				return values === valueToCheck;
			case "isblank":
				return !values;
			case "isnot":
				return values !== valueToCheck;
	...
	const buildConditionExpression = (conditionsData: any[], whenSettings: any, idVal: any) => {
		...
		conditions.forEach((condition, index) => {
			let conditionResult = doFinalLogic(
				condition,
				condition.values.selectedList,
				condition.subList?.selectedList[0]?.id,
				idVal
			);
			if (index === 0) {
				...
				let operator =
					conditions[1]?.mainList?.selectedList[0]?.label === "And" ? "&&" : "||";
	...
	const doFinalLogic = (data: any, value: any, checkId: any, id: any) => {
		...
		if (type === "radio" || type === "dropdown") {
			switch (conditionCheck) {
				case "isfilled":
					isFinalCheck = getValues(value.toString()) ? true : false;
					break;
				case "is":
					isFinalCheck = value[0] === getValues(checkId.toString()) ? true : false;
					break;
				case "isblank":
					isFinalCheck = getValues(value.toString()) ? false : true;
					break;
				case "isnot":
					isFinalCheck = value[0] === getValues(checkId.toString()) ? false : true;
					break;
			}
		} else {
			switch (conditionCheck) {
				case "isfilled":
					isFinalCheck = getValues(checkId.toString()) ? true : false;
					break;
				case "ischecked":
					isFinalCheck = getValues(checkId.toString()) ? true : false;
					break;
				case "is":
					...
					break;
				case "isblank":
					isFinalCheck = getValues(checkId.toString()) ? false : true;
					break;
				case "isnot":
					isFinalCheck = getValues(checkId.toString()) ? false : false;
					break;
				default:
			}
		}
	...
	const getPageBreakValue = () => {
		...
		pageValue = filterPageData[currentSelectedIndex].btnValue
			? filterPageData[currentSelectedIndex].btnValue
			: "Continue";
		return pageValue;
	};
		...
		if (conditionalData.length > 0) {
			conditionalData.forEach((data: any) => {
				...
				if (conditionsData.length === 0 && mainList && conditionalList) {
					...
					const formValues = getValues(checkId.toString());

					if (checkType === "radio" || checkType === "dropdown") {
						finalConditions = evaluateCondition(
							conditionalList,
							formValues,
							valueToCheck
						);
					} else {
						finalConditions = evaluateCheckboxCondition(conditionalList, formValues);
					}

					updateConditionalElements(settings.then, finalConditions);
				} else {
					let finalExpression: any = buildConditionExpression(
						conditionsData,
						whenSettings,
						idVal
					);
	...
	const formElementByItem = (formItem: any, formIndex: number) => {
		const shouldDisplay = shouldDisplayBlock(formData, formItem);
		if (!shouldDisplay) return null;
		const currErrorMsg = errors[formItem.id]?.message;
		switch (formItem?.fieldType) {
					...
					<>
						<Box
							...
							className={`${
								isShowOneByOne
									? isForward
										? "preview-form-item forward-dir"
										: "preview-form-item backward-dir"
									: ""
							}`}
							key={formItem.id}
						>
							...
							<Typography
								className="preview-label-desc-title"
								dangerouslySetInnerHTML={{
									__html:
										formItem.description && formItem.description.length
								...
								InputProps={{
									endAdornment: (
										<InputAdornment position="start">
											<VisibilityIcon
												onMouseOver={() => showActValue(formItem.id)}
												onMouseOut={() => showStarValue(formItem.id)}
												sx={{ cursor: "pointer" }}
											/>
										</InputAdornment>
									),
							...
							{isShowOneByOne && (
								<Box className="preview-label-error">
									{currErrorMsg && isSubmitted ? (
										<div>{`${currErrorMsg}`}</div>
									) : null}
								</Box>
							)}
						</Box>
								...
								<TextField
									...
									onChange={(e: any) => {
										if (/^\s/.test(e.target.value)) e.target.value = "";
											...
											setValue(formItem?.id?.toString(), e?.target?.value, {
												shouldValidate: true,
											});
											setIdVal(formItem.id);
										}
						...
						>
							...
							<Box>
								<AddressComponent
									...
									errorObj={
										errors[formItem.id] && isSubmitted
											? errors[formItem.id]
											: ""
									}
									isFromPreview={true}
							...
							{isShowOneByOne && (
								<Box className="preview-label-error">
									{typeof errors[formItem.id] !== "object" &&
									currErrorMsg &&
									isSubmitted ? (
										<div>{`${currErrorMsg}`}</div>
									) : null}
								...
								return (
									<span key={index}>
										...
										<label htmlFor={formItem.id + "yes" + index}>
											...
											>
												<Box
													className="mult-cancel-box"
													sx={{ position: "relative" }}
												>
													<Box sx={{ display: "flex", width: "95%" }}>
														<Typography className="multi-opt">
															{choice.character.toUpperCase()}
														</Typography>
													...
													{(
														getValues(formItem.id?.toString()) || []
			...
			case "checkbox":
				return (
					<>
						<Box
							sx={{ mb: 2 }}
							className="checkbox-container preview-form-item"
							key={formItem.id}
						>
							<Box sx={{ position: "relative" }}>
								<Box sx={{ display: "flex" }}>
							...
							<div key={formItem.checkboxOption.id}>
								<input
									...
									{...register(formItem.id.toString())}
									checked={getValues(formItem.id?.toString())}
									onChange={(e) =>
										setValue(
								...
								/>
								<label
									htmlFor={`checkbox_${formItem.id}_${formItem.checkboxOption.id}`}
								>
					...
					<>
						<Box
							className={
								isShowOneByOne
									? isForward
										? "preview-display-text preview-form-item forward-dir"
										: "preview-display-text preview-form-item backward-dir"
									: "preview-display-text preview-form-item"
							}
							dangerouslySetInnerHTML={{ __html: formItem.htmlContent }}
							sx={{
								"& p": {
									margin: 0,
								},
						...
						>
							...
							<TextField
								...
								onChange={(event) => {
									...
									setValue(formItem.id?.toString(), formattedValue, {
										shouldValidate: true,
									});
									setFlag((prev) => !prev);
									setIdVal(formItem.id);
										...
										<InputAdornment position="end">
											{!formItem.isPhoneVerified ? (
												<Button
													...
													disabled={
														!getValues(formItem.id.toString()) ||
														getValues(formItem.id.toString())?.length <
															12 ||
														errors[formItem.id] !== undefined
													}
												>
													Verify
												</Button>
							...
							>
								<Box className="verify-code-modal">
									<VerifyCode
										handleCloseVerifyCodeModal={handleCloseVerifyCodeModal}
										otpToVerify={otpVal}
										phoneValue={modalValue}
										formId={modalFormId}
					...
					<>
						<Box
							className={
								isShowOneByOne
									? isForward
										? "yes-preview-container forward-dir"
										: "yes-preview-container  backward-dir"
									: "yes-preview-container"
							}
							sx={{ mb: 2 }}
							key={formItem.id}
						>
							...
							{formItem?.choices?.map((choice: any, index: number) => {
								return (
									<Fragment key={index}>
										...
										<label htmlFor={formItem.id + "yes" + index}>
											<Box
												className="yes-preview-box"
												sx={{ position: "relative" }}
											>
												<Box className="yes-preview-box-child">
													{choice.value.charAt(0)}
												</Box>

												<Box className="yes-preview-box-text">
													{choice.value}
												</Box>
												{getValues(formItem.id.toString()) ===
													choice.value && (
			...
			case "divider":
				return (
					<>
						<Box
							sx={{ borderBottom: "1px solid grey", width: "100%", mb: 2 }}
							key={formItem.id}
						></Box>
					</>
				);
			...
			case "embedanything":
				...
				if (value.endsWith(".pdf")) {
					...
					anySourceContent = <img src={value} alt="allImage" className="preview-emd" />;
							...
							<Stack>
								<Stack sx={{ mt: "10px" }}>
									<Rating
										...
										onChange={(_, newValue) => {
											setIdVal(formItem.id);
											setValue(formItem.id?.toString(), newValue, {
												shouldValidate: true,
											});
										}}
									/>
					...
					<>
						...
						>
							...
							<RankingComponent
								isFromPreview={true}
								field={formItem}
								changeHandlerFn={setValue}
								getRankValues={getRankValues}
								optionsList={getValues(formItem.id.toString())}
							/>
								...
								>
									{formItem.choices?.map((item: any, index: number) => (
										<div
											...
											style={{
												backgroundColor:
													getValues(formItem.id.toString()) == item
														? "rgb(4 69 175 / 30%)"
														: "",
												width: "100%",
											}}
										>
											<input
												type="radio"
												id={formItem.id + "id" + index}
												{...register(formItem.id.toString())}
												value={item}
											/>
											<label
												htmlFor={formItem.id + "id" + index}
												className="opinion-child-container preview-label"
											>
												{item}
											</label>
										</div>
								...
								>
									{opinionScaleArray.map((item: any, index: number) => (
										...
										>
											...
											/>
											<label
												htmlFor={formItem.id + "net" + index}
												className="opinion-child-container net-prometer"
											>
												{item}
											</label>
										</div>
					...
					<>
						...
						>
							...
							<AutocompleteComponent
								...
								dropdownValue={getValues(formItem.id.toString())}
							/>

							{isShowOneByOne && (
								<Box sx={{ fontWeight: "400" }} className="preview-label-error">
									{currErrorMsg && isSubmitted ? (
										<div>{`${currErrorMsg}`}</div>
									) : null}
								</Box>
					...
					<>
						...
						>
							...
							<FileUploadComponent
								...
								field={formItem}
								indexVal={formIndex}
								fileData={getValues(formItem.id.toString())}
							/>
	...
	const onError = (errors: any) => {
		setIsSubmitted(true);
		// if isShowOneByOne = true i.e.fields getting shown one by one
		if (isShowOneByOne) {
			let errorMsg = errors?.[selectedId]?.message || "";
			if (
				!errorMsg &&
				(errors?.[selectedId]?.hasOwnProperty("full_address") ||
					errors?.[selectedId]?.hasOwnProperty("state") ||
					errors?.[selectedId]?.hasOwnProperty("city"))
			) {
				errorMsg = errors?.[selectedId];
			}
			if (errorMsg) {
				setError(selectedId, {
					type: "manual",
		...
		onSaveChangesCalled(errors);
	};

	return (
		<div
			className={`componentView ${isShowOneByOne ? "" : "showAllFields"}`}
			style={{ paddingTop: isPreview ? 0 : "10px" }}
		>
			<div
				className={`moduleView ${displayView} ${
					isMobileSafari ? "isMobileSafari" : "isNotMobileSafari"
				}`}
			>
				<Grid
					container
				...
				>
					<Grid size={12}>
						<form
							...
							className={`previewContainter`}
						>
							{isShowOneByOne && (
								<Box>
									{!isPreview ? (
										<Typography className="formTitle">{formName}</Typography>
									) : null}
							...
							<div className="onlyFoe">
								<Grid
									container
									direction="column"
									className={`formInputsGrid ${
										isForward ? "forward-dir" : "backward-dir"
									}`}
								>
									{currPageElements &&
										currPageElements?.map(
											(formItem: any, formIndex: number) => {
												return (
													<div
														className={`${
															currentSelectedIndex === formIndex ||
															!isShowOneByOne
																? ""
																: "d-none"
														}`}
									...
									{isShowOneByOne &&
										currentSelectedIndex == pageData.length &&
										!isFormSubmitted && (
											<Button
												size="small"
												variant="contained"
									...
									{isFormSubmitted && isShowOneByOne ? (
										<Grid style={{ margin: "0 2em" }}>
											<Grid>
												...
												touch with you shortly for further proceedings.
											</Grid>
											<Grid>
												Please wait. You will be redirected in {counter}{" "}
												seconds
											</Grid>
										</Grid>
									) : null}
							...
							{displayView === "mobile" && !isFormSubmitted ? (
								<Grid
									className={`${isForward ? "forward-dir" : "backward-dir"}`}
									container
									direction="row"
								...
								>
									{isShowOneByOne ? (
										currentSelectedIndex === pageData.length - 1 ? (
											<Button
												size="small"
												variant="contained"
												type="submit"
									...
									{!isShowOneByOne ? (
										<>
											<Button variant="contained" type="submit">
												{currentSelectedIndex < arrayList.length - 1 &&
												arrayList[currentSelectedIndex + 1]?.length > 0
													? getPageBreakValue()
													: "Submit"}
											</Button>
										</>
												...
												>
													...
													<IconButton
														...
														disabled={
															currentSelectedIndex ===
																formData.length - 1 ||
															currentSelectedIndex === formData.length
														}
														type="submit"
														size="small"
													>
